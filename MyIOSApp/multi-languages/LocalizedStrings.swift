
// Generated File.

import Foundation

class LocalizedStrings {

    static var CANCEL: String {
        return LocalizationHelper.shared.value(forKey: "CANCEL")
    }

    static var OK: String {
        return LocalizationHelper.shared.value(forKey: "OK")
    }

    static var I_GOT_IT: String {
        return LocalizationHelper.shared.value(forKey: "I_GOT_IT")
    }

    static var CLOSE: String {
        return LocalizationHelper.shared.value(forKey: "CLOSE")
    }

    static var DONE: String {
        return LocalizationHelper.shared.value(forKey: "DONE")
    }

    static var SUCCESS: String {
        return LocalizationHelper.shared.value(forKey: "SUCCESS")
    }

    static var FAIL: String {
        return LocalizationHelper.shared.value(forKey: "FAIL")
    }

    static var NOTICE: String {
        return LocalizationHelper.shared.value(forKey: "NOTICE")
    }

    static var JUMP: String {
        return LocalizationHelper.shared.value(forKey: "JUMP")
    }

    static var SCAN: String {
        return LocalizationHelper.shared.value(forKey: "SCAN")
    }

    static var ABLUM: String {
        return LocalizationHelper.shared.value(forKey: "ABLUM")
    }

    static var TODO_LIST: String {
        return LocalizationHelper.shared.value(forKey: "TODO_LIST")
    }

    static var PASTEBOARD_HISTORY: String {
        return LocalizationHelper.shared.value(forKey: "PASTEBOARD_HISTORY")
    }

    static var UTILITIES: String {
        return LocalizationHelper.shared.value(forKey: "UTILITIES")
    }

    static var CONTACT: String {
        return LocalizationHelper.shared.value(forKey: "CONTACT")
    }

    static var IMAGE: String {
        return LocalizationHelper.shared.value(forKey: "IMAGE")
    }

    static var VIDEO: String {
        return LocalizationHelper.shared.value(forKey: "VIDEO")
    }

    static var LOCAL_STORAGE: String {
        return LocalizationHelper.shared.value(forKey: "LOCAL_STORAGE")
    }

    static var ABOUT: String {
        return LocalizationHelper.shared.value(forKey: "ABOUT")
    }
}